...
- возврат монитора (конец `synchronized`, метод unlock) и всё, что перед ним в том же потоке:
  - таким образом, оптимизатор может заносить строки в блок синхронизации, но не наружу;
...
- запись в `volatile`-переменную и последующее считывание её же:
    - Другими словами, `volatile`-запись делает с памятью то же, что и возврат монитора, а чтение - то же, что захват.
      Следовательно, если один поток записал в volatile-переменную, а второй обнаружил это, всё, что предшествует
      записи, выполняется раньше всего, что идёт после чтения; см. иллюстрацию;
...
Пессимистичная блокировка - это стратегия, при которой:
...
- запись была успешно обновлена (успешно попала под условие `where`):
...
- запись не была обновлена (не попала под условие `where`):
    - значит где-то между нашим чтением и записью данных в БД кто-то изменил данные:
        - неблагоприятный исход, требуется либо снова вычитать запись из БД и повторить все выполненные действия, либо
...
Мы можем немного улучшить потоко-безопасную реализацию класса `Counter`, используя вместо этого другой объект в качестве
блокировки монитора:
...
![Deadlock in life](images/deadlock_cars.png)

![Deadlock in programming](images/deadlock_threads.png)
...
#### Как избежать deadlock / livelock?

1. отпускать захваченные мьютексы в обратном захвату порядке, т.е. руководствуйтесь логикой "первый захвачен - последний отпущен";
2. соблюдать один и тот же порядок захвата мьютексов.

Подробнее см. [здесь](https://habr.com/ru/companies/nordavind/articles/176541/).
